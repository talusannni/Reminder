...
export default class DataTable extends Component {
    ...
    if (this.state.entities.data.length) {
      return this.state.entities.data.map(user => {
        return <tr key={ user.id }>
          ...
          <td>
            <a href={"/reminder/" + user.id + "/edit"}><i className="far fa-edit"></i></a>&nbsp;
            <a href="#" onClick={() => {if (window.confirm('Are you sure you wish to delete ' + user.name + '?')) this.removeData(user.id)}}><i className="far fa-trash-alt"></i></a>
            </td>
        </tr>
      })
    } else {
      return <tr>
        <td colSpan={this.props.columns.length} className="text-center">No Reminders Found.</td>
      </tr>
    }
  }
  ...
  removeData(id) {
    ...
    axios.delete(url).then(res => {
        const del = this.state.entities.data.filter(row => id !== row.id)
